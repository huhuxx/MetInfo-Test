...
//48、ID208 内容管理-资料下载-访问量验证  
	...
	public void visitNum() throws InterruptedException {
		//进入内容管理-资料下载界面
		this.openDataDowmloadSetting();
		//变量count记录“产品使用说明书syj”的访问量
		String beforeVisitNum=webtest.getElement("xpath=//a[text()='产品使用说明书syj']/../following-sibling::td[1]").getText();
		System.out.println(beforeVisitNum);
		int count=Integer.parseInt(beforeVisitNum);
		System.out.println("访问操作前的访问量为："+count);	
		//关闭界面
		webtest.click("xpath=//button[text()='关闭']");
		...
		webtest.click("xpath=//span[text()='资料下载']");
		Thread.sleep(2000);
		webtest.click("xpath=//span[text()='产品使用说明书syj']");
		webtest.leaveFrame();
		//进入内容管理-资料下载界面，验证访问量是否增加
		this.openDataDowmloadSetting();
		String afterVisitNum=webtest.getElement("xpath=//a[text()='产品使用说明书syj']/../following-sibling::td[1]").getText();
		System.out.println("访问操作后的访问量为："+Integer.parseInt(afterVisitNum));
...
//49、	ID209 内容管理-资料下载-复制
	...
	public void dataCopy() throws InterruptedException {
		...
		webtest.enterFrame1("xpath=//iframe[@src='http://localhost:98/index.php?lang=cn&pageset=1']");
		webtest.click("xpath=//span[text()='资料下载']");
		List<WebElement> beforeCopy_dataList=webtest.getElementsList("xpath=//span[text()='产品使用说明书syj']");
		System.out.println("复制前，产品使用说明书syj的数量为："+beforeCopy_dataList.size());
		Assert.assertEquals(webtest.isElementPresent("xpath=//span[text()='产品使用说明书syj']"),true );
		webtest.leaveFrame();
		//进入内容管理-资料下载界面
		this.openDataDowmloadSetting();
		//选中“产品说明书syj”，点击复制，选择“简体中文，点击弹出的“资料下载”
		webtest.click("xpath=//a[text()='产品使用说明书syj']/../preceding-sibling::td[1]/div");
		...
		webtest.click("xpath=//span[text()='资料下载']");
		Thread.sleep(2000);
		List<WebElement> afterCopy_dataList=webtest.getElementsList("xpath=//span[text()='产品使用说明书syj']");
		System.out.println("产品使用说明书syj的数量为："+afterCopy_dataList.size());
...
//50、ID213 内容管理-资料下载-添加内容  
	...
	public void addData() throws InterruptedException {
		...
		//保存
		List<WebElement> submitBtns=webtest.getElementsList("xpath=//button[text()='保存']");
		System.out.println("保存按钮个数："+submitBtns.size());
		webtest.click(submitBtns.get(1));
		Thread.sleep(3000);
		webtest.click("xpath=//button[text()='关闭']");
